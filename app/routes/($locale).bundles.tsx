...
function ProductBundleCard({
  ...
  const [selectedColor, setSelectedColor] = useState(initialColor || getFirstColor(selectedProduct));
  const [selectedSize, setSelectedSize] = useState(initialSize || getFirstSize(selectedProduct));
  ...
  const colorOptions = selectedProduct?.options?.find((o) => o.name.toLowerCase() === 'color')?.optionValues.map((v) => v.name) || [];
  const sizeOptions = selectedProduct?.options?.find((o) => o.name.toLowerCase() === 'size')?.optionValues.map((v) => v.name) || [];
  ...
  return (
    <div className="flex flex-col border border-gray-100 p-3">
      ...
      <div className="w-full aspect-square mb-3 bg-gray-100 max-h-48">
        {image ? (
          <img src={image} alt="Selected variant" className="w-full h-full object-cover" />
        ) : (
            ...
            return (
              ...
              >
                <div
                  ...
                  style={{ backgroundColor: swatchColor, padding: 0, margin: 0, display: 'block' }}
                >
                  {oos && (
...
export async function loader({ context }: any) {
  ...
  return { polos, denims, cocktailsBabyTee };
}

export default function BundlesPage() {
  const { polos, denims, cocktailsBabyTee } = useLoaderData<typeof loader>();
  const { open } = useAside();

  // --- 1. Denim + Polo Bundle ---
  const [selectedDenim, setSelectedDenim] = useState(denims[0] || null);
  const [selectedDenimVariantId, setSelectedDenimVariantId] = useState(selectedDenim?.variants.nodes[0]?.id || '');
  const [selectedPolo, setSelectedPolo] = useState(polos[0] || null);
  const [selectedPoloVariantId, setSelectedPoloVariantId] = useState(selectedPolo?.variants.nodes[0]?.id || '');

  // --- 2. 2 Polos Bundle ---
  const [selectedPolo1, setSelectedPolo1] = useState(polos[0] || null);
  const [selectedPolo1VariantId, setSelectedPolo1VariantId] = useState(selectedPolo1?.variants.nodes[0]?.id || '');
  const [selectedPolo2, setSelectedPolo2] = useState(polos[1] || polos[0] || null);
  const [selectedPolo2VariantId, setSelectedPolo2VariantId] = useState(selectedPolo2?.variants.nodes[0]?.id || '');
  ...
  function getPriceInfo(variant) {
    ...
    return {
      ...
      compareAt: variant.compareAtPrice ? parseFloat(variant.compareAtPrice.amount) : null,
    ...
    <div className="max-w-3xl mx-auto py-10 px-4">
      ...
      <div className="flex flex-col gap-10">
        ...
        <div className="mb-4 border border-gray-200 p-6">
          <h2 className="text-2xl font-medium text-black mb-2">Denim + Polo Bundle – 10% Off!</h2>
          <p className="mb-6 text-gray-700">Add a matching piece to complete your look and save 10%.</p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <ProductBundleCard
              ...
              onChange={(product, color, size) => {
                ...
                setSelectedDenimVariantId(product?.variants.nodes.find(v =>
                  v.selectedOptions.some(o => o.name.toLowerCase() === 'color' && o.value === color) &&
                  v.selectedOptions.some(o => o.name.toLowerCase() === 'size' && o.value === size)
                )?.id || '');
              }}
              initialColor={selectedDenim?.options?.find(o => o.name.toLowerCase() === 'color')?.optionValues.find(v => v.name === selectedDenim?.options?.find(o => o.name.toLowerCase() === 'color')?.optionValues[0]?.name)?.name}
              initialSize={selectedDenim?.options?.find(o => o.name.toLowerCase() === 'size')?.optionValues.find(v => v.name === selectedDenim?.options?.find(o => o.name.toLowerCase() === 'size')?.optionValues[0]?.name)?.name}
            />
            {/* Polo selection */}
            <ProductBundleCard
              ...
              onChange={(product, color, size) => {
                ...
                setSelectedPoloVariantId(product?.variants.nodes.find(v =>
                  v.selectedOptions.some(o => o.name.toLowerCase() === 'color' && o.value === color) &&
                  v.selectedOptions.some(o => o.name.toLowerCase() === 'size' && o.value === size)
                )?.id || '');
              }}
              initialColor={selectedPolo?.options?.find(o => o.name.toLowerCase() === 'color')?.optionValues.find(v => v.name === selectedPolo?.options?.find(o => o.name.toLowerCase() === 'color')?.optionValues[0]?.name)?.name}
              initialSize={selectedPolo?.options?.find(o => o.name.toLowerCase() === 'size')?.optionValues.find(v => v.name === selectedPolo?.options?.find(o => o.name.toLowerCase() === 'size')?.optionValues[0]?.name)?.name}
            />
          </div>
          {/* Price calculation */}
          <div className="mt-6 text-center">
            {(() => {
              const denimVariant = getVariant(selectedDenim, selectedDenimVariantId);
              const poloVariant = getVariant(selectedPolo, selectedPoloVariantId);
              ...
              return (
                <div className="flex items-center justify-center gap-2">
                  <span className="text-gray-500 line-through">{original.toFixed(2)} {denimPrice.currency}</span>
                  <span className="text-xl font-bold">{discounted.toFixed(2)} {denimPrice.currency}</span>
        ...
        {/* 2. 2 Polos Bundle */}
        <div className="mb-4 border border-gray-200 p-6">
          <h2 className="text-2xl font-medium text-black mb-2">2 Polos Bundle – 10% Off!</h2>
          <p className="mb-6 text-gray-700">Pick any 2 polos (choose color and size for each) and get 10% off.</p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {[0, 1].map((idx) => (
              <ProductBundleCard
                key={idx}
                ...
                onChange={(product, color, size) => {
                  if (idx === 0) {
                    ...
                    setSelectedPolo1VariantId(product?.variants.nodes.find(v =>
                      v.selectedOptions.some(o => o.name.toLowerCase() === 'color' && o.value === color) &&
                      v.selectedOptions.some(o => o.name.toLowerCase() === 'size' && o.value === size)
                    )?.id || '');
                  } else {
                    setSelectedPolo2(product);
                    setSelectedPolo2VariantId(product?.variants.nodes.find(v =>
                      v.selectedOptions.some(o => o.name.toLowerCase() === 'color' && o.value === color) &&
                      v.selectedOptions.some(o => o.name.toLowerCase() === 'size' && o.value === size)
                    )?.id || '');
                  }
                }}
                initialColor={idx === 0 ? selectedPolo1?.options?.find(o => o.name.toLowerCase() === 'color')?.optionValues.find(v => v.name === selectedPolo1?.options?.find(o => o.name.toLowerCase() === 'color')?.optionValues[0]?.name)?.name : selectedPolo2?.options?.find(o => o.name.toLowerCase() === 'color')?.optionValues.find(v => v.name === selectedPolo2?.options?.find(o => o.name.toLowerCase() === 'color')?.optionValues[0]?.name)?.name}
                initialSize={idx === 0 ? selectedPolo1?.options?.find(o => o.name.toLowerCase() === 'size')?.optionValues.find(v => v.name === selectedPolo1?.options?.find(o => o.name.toLowerCase() === 'size')?.optionValues[0]?.name)?.name : selectedPolo2?.options?.find(o => o.name.toLowerCase() === 'size')?.optionValues.find(v => v.name === selectedPolo2?.options?.find(o => o.name.toLowerCase() === 'size')?.optionValues[0]?.name)?.name}
              />
            ))}
          </div>
          {/* Price calculation */}
          <div className="mt-6 text-center">
            {(() => {
              ...
              return (
                <div className="flex items-center justify-center gap-2">
                  <span className="text-gray-500 line-through">{original.toFixed(2)} {p1.currency}</span>
                  <span className="text-xl font-bold">{discounted.toFixed(2)} {p1.currency}</span>
                  <span className="text-green-600 text-sm">(Save 10%)</span>
                </div>
              );
            })()}
        ...
        {/* 3. 3 Tops Bundle (Cocktails baby tee) */}
        <div className="mb-4 border border-gray-200 p-6">
          <h2 className="text-2xl font-medium text-black mb-2">3 Tops Bundle – 15% Off!</h2>
          <p className="mb-6 text-gray-700">Pick any 3 tops (choose color and size for each) and get 15% off.</p>
            ...
            {[0, 1, 2].map((idx) => (
              <ProductBundleCard
                ...
                onChange={(product, color, size) => {
                  ...
                  newSelections[idx].variantId = product?.variants.nodes.find(v =>
                    v.selectedOptions.some(o => o.name.toLowerCase() === 'color' && o.value === color) &&
                    v.selectedOptions.some(o => o.name.toLowerCase() === 'size' && o.value === size)
                  )?.id || '';
                  setTopSelections(newSelections);
                }}
                initialColor={topSelections[idx].variantId ? getVariant(cocktailsBabyTee, topSelections[idx].variantId)?.selectedOptions.find(o => o.name.toLowerCase() === 'color')?.value : ''}
                initialSize={topSelections[idx].variantId ? getVariant(cocktailsBabyTee, topSelections[idx].variantId)?.selectedOptions.find(o => o.name.toLowerCase() === 'size')?.value : ''}
              />
            ))}
          </div>
          {/* Price calculation */}
          <div className="mt-6 text-center">
            {(() => {
              const prices = topSelections.map(sel => getPriceInfo(getVariant(cocktailsBabyTee, sel.variantId)));
              const original = prices.reduce((sum, p) => sum + (p.price || 0), 0);
              const discounted = original * 0.85;
              return (
                <div className="flex items-center justify-center gap-2">
                  <span className="text-gray-500 line-through">{original.toFixed(2)} {prices[0]?.currency}</span>
                  <span className="text-xl font-bold">{discounted.toFixed(2)} {prices[0]?.currency}</span>
          ...
          <div className="mt-6">
            <AddToCartButton
              lines={topSelections.map(sel => ({ merchandiseId: sel.variantId, quantity: 1 }))}
              onClick={() => open('cart')}
            >
              <span className="block w-full text-center py-3 px-6 tracking-wide text-base font-medium transition-all duration-200 bg-black text-white hover:bg-gray-800 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-gray-300 disabled:bg-gray-300 disabled:text-gray-500 disabled:cursor-not-allowed">
                Add Bundle to Cart
        ...
        {/* 4. 3 Polos Bundle */}
        <div className="mb-4 border border-gray-200 p-6">
          <h2 className="text-2xl font-medium text-black mb-2">3 Polos Bundle – 15% Off!</h2>
          <p className="mb-6 text-gray-700">Pick any 3 polos (choose color and size for each) and get 15% off.</p>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[0, 1, 2].map((idx) => (
              <ProductBundleCard
                key={idx}
                products={[selectedPolos[idx], selectedPolos[idx], selectedPolos[idx]]}
                ...
                onChange={(product, color, size) => {
                  ...
                  newVariants[idx] = product?.variants.nodes.find(v =>
                    v.selectedOptions.some(o => o.name.toLowerCase() === 'color' && o.value === color) &&
                    v.selectedOptions.some(o => o.name.toLowerCase() === 'size' && o.value === size)
                  )?.id || '';
                ...
                initialColor={selectedPolos[idx]?.options?.find(o => o.name.toLowerCase() === 'color')?.optionValues.find(v => v.name === selectedPolos[idx]?.options?.find(o => o.name.toLowerCase() === 'color')?.optionValues[0]?.name)?.name}
                initialSize={selectedPolos[idx]?.options?.find(o => o.name.toLowerCase() === 'size')?.optionValues.find(v => v.name === selectedPolos[idx]?.options?.find(o => o.name.toLowerCase() === 'size')?.optionValues[0]?.name)?.name}
              />
            ))}
          </div>
          {/* Price calculation */}
          <div className="mt-6 text-center">
            {(() => {
              const prices = selectedPolosVariantIds.map((variantId, idx) => getPriceInfo(getVariant(selectedPolos[idx], variantId)));
              const original = prices.reduce((sum, p) => sum + (p.price || 0), 0);
              const discounted = original * 0.85;
              return (
                <div className="flex items-center justify-center gap-2">
          ...
          <div className="mt-6">
            <AddToCartButton
              lines={selectedPolosVariantIds.map(variantId => ({ merchandiseId: variantId, quantity: 1 }))}
              onClick={() => open('cart')}
            >
              <span className="block w-full text-center py-3 px-6 tracking-wide text-base font-medium transition-all duration-200 bg-black text-white hover:bg-gray-800 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-gray-300 disabled:bg-gray-300 disabled:text-gray-500 disabled:cursor-not-allowed">
                Add Bundle to Cart
